...
export class AssetTransactionsService {
  ...
  async createRequest(
    ...
    const { assetId, fromUserId, toUserId, departmentId, officeId, note } =
  ...
  async getConfirmRequest(assetId: string) {
    const assetTransaction = await this.prisma.assetTransaction.findFirst({
      where: {
        assetId,
        role: TransactionRole.TO,
  ...
  async confirmRequest(assetId: string, signature?: Express.Multer.File) {
    try {
      const assetTransaction = await this.prisma.assetTransaction.findFirst({
        where: {
          assetId,
      ...
      const returnAssetTransaction = await this.prisma.assetTransaction.update({
        where: { id: assetTransaction.id },
        data: {
